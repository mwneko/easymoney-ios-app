//
//  SpendingCategoryViewModel.swift
//  EasyMoneyCoreData
//
...
class SpendingCategoryViewModel {
    ...
    // set 'SpendingCategory' array - for response data
    ...
    private let spendingCategoryURL = "https://yjpihxuk36.execute-api.ap-southeast-2.amazonaws.com/iPhone_SE_Assignment/getspendingcategories"
        ...
        let task = session.dataTask(with: request, completionHandler: {
            ...
            if let error = downloadError{
                ...
                if let dataResponse = data{
                    if let decodeResponse = try? JSONDecoder().decode([SpendingCategory].self, from: dataResponse){
                        for category in decodeResponse {
                            let spending = SpendingCategory.init(name: category.name, sub_name: category.sub_name, SC_ID: category.SC_ID)
                            self.categories.append(spending)
                        }
                        complete()
                    }
                }
