...
class SpendingController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate {
    // link to the view UI
    @IBOutlet weak var spendingTitle: UITextField!
    @IBOutlet weak var spendingAmount: UITextField!
    @IBOutlet weak var spendingDescription: UITextView!
    @IBOutlet weak var spendingCollectionView: UICollectionView!
    ...
    private let spendingCategoryViewModel = SpendingCategoryViewModel()
    ...
    override func viewDidLoad() {
        ...
        self.spendingCollectionView.dataSource = self
        self.spendingCollectionView.delegate = self
        ...
        self.tabBarController!.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(submitSpending))
        
        // clear 'spendingCollectionView' backgroud color
        self.spendingCollectionView.backgroundColor = UIColor.clear
        
        // set 'spendingTitle' delegate
        spendingTitle.delegate = self
    }
    
    override func viewWillAppear(_ animated: Bool) {
        // reload collection view data
        self.spendingCollectionView.reloadData()
        ...
        // if the vertical trati is compact, change to regular
    ...
    private func submitCheck() -> String {
        if(spendingTitle.text == ""){
            return SystemMessage.titleError.rawValue
        }else if(spendingAmount.text == ""){
            return SystemMessage.amountError.rawValue
        }else if(Double(spendingAmount.text!)! > 9999999){
            return SystemMessage.amountTooLarge.rawValue
        }else if(spendingCategoryViewModel.selectCategory == nil){
    ...
    @objc func submitSpending(){
        ...
        if(checkResult == SystemMessage.success.rawValue){
            ...
            let result = BillViewModel().addBill(
                title: spendingTitle.text!,
                amount: spendingAmount.text!,
                billDescription: spendingDescription.text!,
                type: "Spending",
                category: spendingCategoryViewModel.selectCategory!,
                subcategory: spendingCategoryViewModel.selectSubcategory!,
            ...
            if(result){
                ...
                alter = UIAlertController(title: SystemMessage.successTitle.rawValue, message: SystemMessage.addSpendingSuccess.rawValue, preferredStyle: .alert)
                ...
                spendingDescription.text = "Please enter bill description"
                
                // reset the select category and subcategory
                spendingCategoryViewModel.setCategories(category: nil, subcategory: nil)
                
                // reload the data
                self.spendingCollectionView.reloadData()
            }else{
                // if faced error, aler error
                alter = UIAlertController(title: SystemMessage.errorTitle.rawValue, message: SystemMessage.addSpendingFailed.rawValue, preferredStyle: .alert)
                alter.addAction(cancel)
                self.present(alter, animated: true, completion: nil)
            }
        }else{
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return spendingCategoryViewModel.getCount()
    }
    
    // set data source
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        // register cell
        collectionView.register(CategoryCell.self, forCellWithReuseIdentifier: "SpendingCell")
        
        // create cell
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SpendingCell", for: indexPath) as! CategoryCell
        
        /*
         get the data from 'SpendingCategoryViewModel'
         set the value to the cell
         */
        cell.categoryLabel.text = spendingCategoryViewModel.getSubcategory(index: indexPath.row)
        cell.categroy = spendingCategoryViewModel.getCategory(index: indexPath.row)
        cell.subcategory = spendingCategoryViewModel.getSubcategory(index: indexPath.row)
    ...
    // set the style when the cell be selected
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        // change the values when the cell be selected
        spendingCategoryViewModel.setCategories(category: cell.categroy, subcategory: cell.subcategory)
    }
    
    // set the style when the cell be deselected
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let collection = self.spendingCollectionView{
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return self.unitViewModel.getCount()
    }
