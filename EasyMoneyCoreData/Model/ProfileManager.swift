...
class ProfileManager {
    ...
    func fetchProfiles() -> [Profile] {
        ...
        do{
            // fetch bill
            profiles = try self.context.fetch(Profile.fetchRequest())
            return profiles
        }catch{
            print(error.localizedDescription)
            return profiles
